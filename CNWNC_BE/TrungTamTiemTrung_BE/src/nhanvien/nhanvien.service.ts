...
export class NhanVienService {
    ...
    async create(dataNhanVien: nhanvien): Promise<any> {
        var passWordCrypto = crypto.AES.encrypt(dataNhanVien.password, '123').toString();
        try {
            ...
            dataNhanVien.password = passWordCrypto;
    ...
    async changePass(data): Promise<any> {
        try {
            var passWordCrypto = crypto.AES.encrypt(data.newPass, '123').toString();
            let nv = await this.nhanVienRepo.findOne({ username: data.username })
            nv.password = passWordCrypto;
